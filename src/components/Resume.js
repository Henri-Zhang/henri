...
class Resume extends Component {
  ...
  updateLangToggle(activeSlide) {
    ...
    this.refs.langToggle.childNodes.forEach(item => {
      if (item.className.match(new RegExp("(\\s|^)" + styles.active + "(\\s|$)"))) {
  ...
  render() {
    ...
    fullPageOptions.slides = [
      <Slide style={{backgroundColor: this.slidesColor[0]}}>
        <Raining ref="raining" />
        <div className={classNames('container', styles['basic-info'], styles['absoulte-center'])}>
          <div className="row">
            <div className="col-md-3">
            </div>
            <div className={classNames('col-md-3', styles['photo-container'])}>
              <img className={styles.photo} src={require("./../asserts/images/resume_photo.jpg")} alt={intl.get('photo').defaultMessage('photo')} />
            </div>
            <div className={classNames('col-md-3', styles.names, styles['text-shadow'])}>
              <div>
                <h1>张  恒</h1>
                <h2>Henri Zhang</h2>
              </div>
          ...
          <div className={classNames('row', styles.infos)}>
            <div className="col-md-3">
              <img className={styles.icon} src={require('./../asserts/icons/Resume/calendar.svg')} alt={intl.get('birthday').defaultMessage('birthday')}/>
              <span className={styles.info}>1994.07.18</span>
            </div>
            <div className="col-md-3">
              <img className={styles.icon} src={require('./../asserts/icons/Resume/location.svg')} alt={intl.get('location').defaultMessage('location')}/>
              <span className={styles.info}>{intl.get('shanghai').defaultMessage('Shanghai')}</span>
            </div>
            <div className="col-md-3">
              <img className={styles.icon} src={require('./../asserts/icons/Resume/phone.svg')} alt={intl.get('phone').defaultMessage('phone')}/>
              <span className={styles.info}>15755503230</span>
            </div>
            <div className="col-md-3">
              <a href="mailto:henrizhang@henri.ren">
                <img className={styles.icon} src={require('./../asserts/icons/Resume/email.svg')} alt={intl.get('email').defaultMessage('email')}/>
      ...
      <Slide style={{backgroundColor: this.slidesColor[1]}} className={styles['educational-background']}>
        <video className={styles['background-video']} loop="loop" autoPlay muted="muted">
          <source src="https://henri.oss-cn-hangzhou.aliyuncs.com/resume/education_background.mov" type="video/mp4" />
      ...
      <Slide style={{backgroundColor: this.slidesColor[2]}} className={styles['project-experience']}>
        <ProjectExperience
          className={styles['absoulte-center']}
      ...
      <Slide style={{backgroundColor: this.slidesColor[3]}} className={styles['professional-skills']}>
        <div className={styles['absoulte-center']}>
          <h1 className={styles['text-shadow']}>{intl.get('professional-skills').defaultMessage('Professional Skills')}</h1>
          <SkillsCloud
      ...
      <Slide style={{backgroundColor: this.slidesColor[4]}} className={styles['self-evaluation']}>
        <SelfEvaluation
          className={styles['absoulte-center']}
    ...
    return (
      <div className={styles['resume-container']}>
        <nav className="navbar navbar-default fixed-top">
          <ul className={styles['lang-toggle']} ref="langToggle">
            <li data-value="en" onClick={this.changeLanguage}>
              <img className={styles.flag} src={require('./../asserts/icons/Resume/Britain_flag.svg')} alt="English" />
              <span>English</span>
            </li>
            <li data-value="zh" onClick={this.changeLanguage}>
              <img className={styles.flag} src={require('./../asserts/icons/Resume/China_flag.svg')} alt="中文" />
              <span>中文</span>
            </li>
          </ul>
          <ul>
        ...
        <ul className={styles.anchors} ref="anchors">
          <li>
            <span className={styles.tip}>{intl.get('basic-information').defaultMessage('Basic Information')}</span>
            <a href="#0" data-slideindex="0" onClick={this.locate}> </a>
          </li>
          <li>
            <span className={styles.tip}>{intl.get('educational-background').defaultMessage('Educational Background')}</span>
            <a href="#1" data-slideindex="1" onClick={this.locate}> </a>
          </li>
          <li>
            <span className={styles.tip}>{intl.get('project-experience').defaultMessage('Project experience')}</span>
            <a href="#2" data-slideindex="2" onClick={this.locate}> </a>
          </li>
          <li>
            <span className={styles.tip}>{intl.get('professional-skills').defaultMessage('Professional Skills')}</span>
            <a href="#3" data-slideindex="3" onClick={this.locate}> </a>
          </li>
          <li>
            <span className={styles.tip}>{intl.get('self-evaluation').defaultMessage('Self Evaluation')}</span>
